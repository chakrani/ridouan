...
var olh=new Array();
for(c=0;c<64;c++){
olh[c]=Math.round(256+(512*Math.random()));
}
var interc=0;
var beggs=0;
...
var stylesga="experiment|asia|strings|hiphop|house|ishu|music|hardrock|piano|east|jazz|symfony|country|signal";
var magxrbffstyle=0;
var aramub=1;
var gcount=0;
...
for(t=0;t<xpose.length;t++){
	...
	xpose[t]=Math.round(xpose[t]+(telse[t]*rcxe[t]));
	ypose[t]=Math.round(ypose[t]+(uelse[t]*rcye[t]));
...
for(gtgg=0;gtgg<lines.length;gtgg++){
	fsfgg=lines[gtgg]
	lier=new Array();
	for(qgtg=0;qgtg<fsfgg.length;qgtg++){
		fsfgg[qgtg]=false;
	}

}

for(reeeew=0;reeeew<xxposp.length;reeeew++){
	wetty=xxposp[reeeew];
	
	if(wetty.length>1){
	
	if(wetty[1]>=0 && wetty[1]<sliceheight && wetty[0]>=0 && wetty[0]<sliceheight){
	xwery=wetty[1];
	ywery=wetty[0];
...
if(fwtuo.length<2){
stylesga="experiment|asia|strings|hiphop|house|ishu|music|hardrock|piano|east|jazz|symfony|country|signal";
...
var daaxavb=new Array();
fdb=0;
tegt=0;
var edaaxb=new Array();
for(fcdt=0;fcdt<12;fcdt++){
edaaxb[0]=35;
edaaxb[1]=36;
edaaxb[2]=54;
...
for(vtr=0;vtr<edaaxb.length;vtr++){
        daaxavb[tegt]=edaaxb[vtr];
        tegt++;
}
}
...
if(genre=="ishu" && gcount==4){
        nnotte=edruu(see);
        tmpup=Math.round(oudt*4);
        tmpu=tmpup*50;
}
...
//if(gcount==4){
//va=va+" "+((rl*tmpu)+(tmpu*1))+" Off ch="+chchrul+" n="+nnotte+" v=0<br>";
//}
//else{
va=va+" "+((rl*tmpu)+(tmpu*rb))+" Off ch="+chchrul+" n="+nnotte+" v=0<br>";
//}
...
//setd="MTrk\n"+Math.floor(3/4*tijdje)+" Tempo "+Math.floor(3/4*tempoose)+"\n"+tijdje+" Meta TrkEnd\nTrkEnd\n";
...
//data=data.replace("0 Tempo "+tempoose, "0 Tempo "+tempoose+"\n"+Math.floor(3/4*tijdje)+" Tempo "+vertt);
//data=data.replace("xx Meta", tijdje+" Meta");
